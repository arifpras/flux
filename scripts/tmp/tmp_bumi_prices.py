...
for txt_file in brokers_path.glob('*.txt'):
    ...
    if 'DU KAF' in text:
        for m in re.finditer(r'\b(BUMI)\b\s+([0-9.]+[MBK]?)\s+([0-9,]+)\s+([0-9,]+)', text):
            avg = m.group(4).replace(',', '')
            if avg.isdigit():
                bumi_prices.append((broker, int(avg)))
        continue
    ...
    stocks, bavgs = res
    if 'BUMI' in stocks:
        idx = stocks.index('BUMI')
        if idx < len(bavgs):
            avg = re.sub(r'[^0-9]', '', bavgs[idx])
            if avg:
                bumi_prices.append((broker, int(avg)))

print('BUMI prices by broker (OCR-aligned):')
for broker, price in sorted(bumi_prices, key=lambda x: x[1]):
